...
<body>
	...
	<div id="container">
		...
		<div id="pnlHeader">
				...
				if (getMsgHeadersButton != null) 
				{
					...
					else if (accessToken !=  null)
					{
						...
						if ((statusCode1 == 200 || statusCode1 == 201)
								&& msgContent == null) {
							JSONObject rpcObject = new JSONObject(
									method.getResponseBodyAsString());
							JSONObject mgeHdrList = rpcObject
									.getJSONObject("MessageHeadersList");
							...
							<div>
								<table id="gvMessageHeaders" cellspacing="0" cellpadding="3"
									...
									<tr style="color: White; background-color: #006699;">
										...
										<th scope="col" class="style3">Favorite</th>
									...
									<tr class="style3">
										...
										<%
											if (inHeaders.has("MmsContent")) {
															JSONArray MmsContentArray = inHeaders
																	.getJSONArray("MmsContent");
															for (int x = 0; x < MmsContentArray.length(); x++) {
																JSONObject inMmsContent = MmsContentArray
																		.getJSONObject(x);
										%>
											<tr>
												<td class="style3"></td>
		...
		<%
			} else if (getMsgHeadersButton != null
						&& request.getParameter("HeaderCount") == "") {
		%>
			<div class="errorWide">
		...
		<%
				method.releaseConnection();
						...
						&& request.getParameter("HeaderCount") == ""
						&& request.getParameter("IndexCursor") != "") {
				%><div class="errorWide">
					<strong>ERROR:</strong> Header Count field is empty.<br />
				</div>
				<br />
		<%
				method.releaseConnection();
				} else if ((statusCode1 != 200 || statusCode1 != 201)
						&& msgContent == null) {
		%>
				<div class="errorWide">
		...
		<%
				...
				method.releaseConnection();
					...
					else if (session.getAttribute("errorResponse") != null)
					{
		%>
				<div class="errorWide">
					<strong>ERROR:</strong><br />
					<%=session.getAttribute("errorResponse")%>
				</div>
				<br />
		<%				
					}
					...
					<tbody>
						<tr>
							<td class="cell"><input type="submit" name="msgContent"
								value="Get Message Content" id="Submit1" /></td>
						</tr>
					</tbody>
				</table>
			</div>
		...
		if (msgContent != null) {
				...
				String url = FQDN + "/rest/1/MyMessages/" + MessageId + "/"
						+ PartNumber;
				URL url1 = new URL(FQDN + "/rest/1/MyMessages/" + MessageId
						+ "/" + PartNumber);
				...
				if (statusCode == 200) {
					...
					<%
						...
						byte[] encoded = method.getResponseBodyAsString()
								.getBytes();
						Base64 dat = new Base64();
					%>
					<div id="imagePanel" style="text-align: center;">
					<img
						src="data:<%=type%>;base64,<%=dat.encodeBase64String(method
										.getResponseBody())%>" />
					<br />
					<%
					}
				...
				<%
					method.releaseConnection();
							...
							&& request.getParameter("MessageId") == ""
							&& request.getParameter("PartNumber") != "") {
				%>
					<div class="errorWide">
						<strong>ERROR:</strong> Message Id field is empty.<br />
					</div>
		...
		%>
			<div class="errorWide">
				<strong>ERROR:token error</strong><br />
				<%=session.getAttribute("errorResponse")%>
			</div>
			<br />
		<%	
