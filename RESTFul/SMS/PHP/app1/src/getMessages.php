...
try {
    ...
    foreach ($msgs as $msg) {
        $val = array(
            $msg->getMessageId(), $msg->getSenderAddress(), $msg->getMessage()
        );
        $values[] = $val;
    }
    ...
    $arr = array(
        ...
        'tables' => array(
            array(
                ...
                'values' => array(
                    array(
                        $response->getNumberOfMessages(),
                        $response->getResourceUrl(),
                        $response->getNumberOfPendingMessages(),
                    )
                ),
            ),
            array(
                'caption' => 'Messages',
                'headers' => array('Message ID', 'Sender Address', 'Message'),
                'values' => $values,
            )
        )
    );
