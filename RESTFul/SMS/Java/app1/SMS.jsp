...
<head>
<title>AT&amp;T Sample SMS Application - Basic SMS Service
	Application</title>
...
<body>
	...
	<%
		...
		if (smsId == null)
			smsId = (String) session.getAttribute("smsId");
		if (smsId == null)
			smsId = "";
		session.setAttribute("smsId", smsId);
		String getSmsDeliveryStatus = request
				.getParameter("getSmsDeliveryStatus");
			...
			if (sendSms != null) {
				if (accessToken != null) {
					...
					if ((address.indexOf("-") == 3) && (address.length() == 12))
						address = "tel:" + address.substring(0, 3)
								+ address.substring(4, 7)
								+ address.substring(8, 12);
					else if ((address.indexOf(":") == 3)
							&& (address.length() == 14))
						address = address;
					...
					<tbody>
						<tr>
							<td class="cell"><button type="submit"
									name="getSmsDeliveryStatus">Get Status</button></td>
						</tr>
					</tbody>
				</table>
				...
				if (accessToken != null) {
					...
					if (statusCode == 200) {
						...
						JSONObject deliveryInfoList = new JSONObject(
								jsonResponse.getString("DeliveryInfoList"));
						JSONArray deliveryInfoArray = new JSONArray(
								deliveryInfoList.getString("DeliveryInfo"));
						JSONObject deliveryInfo = new JSONObject(
								deliveryInfoArray.getString(0));
		%>
					<div class="successWide">
...
%>
		...
		<div id="wrapper">
			<div id="content">
				...
				<h2>
					<br />Feature 3: Get Received Messages
				</h2>
				...
				if (accessToken != null) {
					...
					if (statusCode == 200) {
						...
						JSONObject smsList = new JSONObject(
								jsonResponse.getString("InboundSmsMessageList"));
						int numberOfMessagesInBatch = Integer.parseInt(smsList
								.getString("NumberOfMessagesInThisBatch"));
						int numberOfMessagesPending = Integer.parseInt(smsList
								.getString("TotalNumberOfPendingMessages"));
						JSONArray messages = new JSONArray(
								smsList.getString("InboundSmsMessage"));
		%>
		<div class="successWide">
		...
		<div align="center">
			<table style="width: 650px" cellpadding="1" cellspacing="1"
				...
				<thead>
					<tr>
						<th style="width: 100px" class="cell"><strong>Message
								Index</strong></th>
						<th style="width: 275px" class="cell"><strong>Message
								Text</strong></th>
						<th style="width: 125px" class="cell"><strong>Sender
								Address</strong></th>
					</tr>
				</thead>
				<tbody>
