...
<body>
	...
	<%
			...
			if (phoneTextBox != null)
			{
				...
				for(String a : address) 
				{
					...
					else if(a.length()>2 && a.length()<=8 && !a.contains("@"))
					{
						String expression = "[0-15]*[0-9]+$";  
						CharSequence inputStr = a;  
						Pattern pattern = Pattern.compile(expression);  
			...
			if((session.getAttribute("accessToken") == null && request.getParameter("error_description") == null && code.length() == 0 && sendMessageButton != null)) 
		   	{
	...
	<div id="container">
		...
		<div id="wrapper">
			<div id="content">
				<h1>AT&T Sample IMMN Application 1 Basic IMMN Service
					Application</h1>
		...
		<form method="post" name="sendMessageButton"
			...
			action="index.jsp?sendMessageButton=true">
			<div id="navigation">
		...
		<div class="successWide">
			<strong>SUCCESS</strong><br /> <strong>Message ID: <%=escape(ID)%></strong>
		</div>
		<br />
		<%
		...
		%>
		<div class="errorWide">
			<strong>ERROR:</strong><br /> <strong><%=escape(responze)%></strong>
		</div>
		<br />
		<%
			}
