...
load_hook = function(data) {
    // The X-SpeechSubContext is currently only applicable to Gaming
    // so hide it unless the context is shown as Gaming
    var speechContext = function() {
        var val = $('#speechContext').val();
        if (val == "Gaming") {
            $('#formSubContext').removeClass('hidden');
        } else {
            $('#formSubContext').addClass('hidden');
        }
    };
    speechContext();
