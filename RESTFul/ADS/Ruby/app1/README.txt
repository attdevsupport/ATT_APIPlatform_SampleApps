...
1. Configuration
  ...
  NOTE: You MUST select ADS in the list of services under field 'Services' in order to use this sample
  application code. 
...
3. Parameters
   ...
   3) tokens_file       : path where the access token will be saved. Must be writable.

   4) api_key           : set the value as per your registered application 'API key' field value

   5) secret_key        : set the value as per your registered application 'Secret key' field value

   6) adType            : optional parameter used for getting desired ad type
