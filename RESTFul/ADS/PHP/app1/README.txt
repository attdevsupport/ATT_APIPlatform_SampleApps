...
3. Parameters
   ...
   Each application contains a config.php file. It holds the following configurable parameters and defaults 
    ...
    $oauth_file = "/tmp/adsoauthtoken.php"; : Oauth Token persistance location,should be writable by apache server  
    $scope = "ADS";                         : Oauth scope ( ADS ) 
    $Udid                                   : The UDID Parameter is used for ad tracking purpose and should be atleast 30 characters
    $Psedo_zone		                : This parameter specifies the mapping between API Gateway and south bound enabler.
