...
{
    /** \addtogroup SMS_App2
     * Description of the application can be referred at \ref SMS_app2 example
     * @{
     */

    /** \example SMS_app2 sms\app2\Default.aspx.cs
     * \n \n SMS voting application with a server component and a web interface. Application allows users to send SMS messages 
     * to a specified short code (given in the application interface) with the one of the words 'baseball', 'basketball' or 'football'. 
     * The application should count up the votes for each sport and display the results to the user. 
     * This application contains a server element (listner) running constantly and checking for new messages,
     * as well as a web interface to display the results and allow the user to manually trigger an update.
     * \n \n For Registration, Installation, Configuration and Execution, refer \ref Application 
     * \n \n <b>Additional configuration to be done:</b>
     * \n Define the following parameters in web.config file.
     * \li <b>FootBallFilePath</b> - This is mandatory key, which will point to the file, which is used by application to update football votes.
     * \li <b>BaseBallFilePath</b> - This is mandatory key, which will point to the file, which is used by application to update baseball votes.
     * \li <b>BasketBallFilePath</b> - This is mandatory key, which will point to the file, which is used by application to update basketball votes.
     * \li <b>ShortCode</b> - This is mandatory key, which will specify the shortcode of the registered application.
     * 
     * \n Documentation can be referred at \ref SMS_App2 section
    * @{
    */

    #region Aplication Instance Variables
    ...
    {
        int count = 0;
        using (StreamReader streamReader = File.OpenText(Server.MapPath(filePath)))
        {
